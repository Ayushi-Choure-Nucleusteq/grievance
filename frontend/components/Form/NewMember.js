...
function NewMember() {
  ...
  const handleChange = (e) => {
    if (e.target.name === "department") {
      const selectedDept = departments.find(dept => dept.deptName === e.target.value);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.email) {
      ...
      validationErrors.email = 'Email can only contain lowercase and "." and must end with @nucleusteq.com.';
    }

    if (!formData.password) {
      validationErrors.password = "Password cannot be empty.";
    } else if (!checkPassword(formData.password)) {
      validationErrors.password = 'Password must contain an uppercase letter, a lowercase letter, a number, a special character, and be at least 8 characters long.';
    }
  ...
  return (
    <form onSubmit={handleSubmit}>
      <h2>New User</h2>
      {successMessage ? <Popup message={successMessage} color="green"></Popup> : <div className="error-message">{backendError}</div>}
      <div>
        <label>Name:</label>
      ...
      <div>
        <label>Email:</label>
        <input type="email" name="email" value={formData.email} onChange={handleChange} onkeyup={handleSubmit} placeholder="john@nucleusteq.com" />
        {errors.email && <span>{errors.email}</span>}
      </div>

      <div>
        <label>Password:</label>
        <input type="password" name="password" value={formData.password} onChange={handleChange} placeholder="Enter a strong password" />
      ...
      <div>
        ...
        <select name="department" value={formData.department.deptName} onChange={handleChange}>
          <option value="" disabled>Select Department</option>
          {departments.map((dept) => <option key={dept.id} value={dept.deptName}>{dept.deptName}</option>)}
        </select>
