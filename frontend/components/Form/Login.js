...
const API_URL = 'http://localhost:8000/api/member/login';
  ...
  const validatePassword = (e) => {
    ...
    if (values.length < 8 || !hasNumber.test(values) || !hasLowercase.test(values) || !hasUppercase.test(values) || !hasSpecialCharacter.test(values)) {
      setPasswordError('Password must be at least 8 characters long, include at least one number, one lowercase letter, and one uppercase letter.');
    } else {
      setPasswordError('');
    }
  }
  ...
  const handleApiCall = async (data) => {
    try {
      ...
      if (response?.status === 202) {
        ...
        sessionStorage.setItem('loginData', JSON.stringify(dataToStore));
    ...
    <div className="login-container">
      ...
      <div className="login_container">
        ...
        {/* {loginError? <Popup message= {loginError} color="red"></Popup>:""} */}
          {loginError && <div className="error-popup" onClick={() => setLoginError('')}>{loginError}</div>}
          {/* <h2>Login</h2> */}
          <h1>{props.msg ? props.msg : ''}</h1>
          <form onSubmit={handleSubmit} className='login_form'>
            <label htmlFor="username">Username<span className='star'>*</span></label>
            <input id="username" type="text" placeholder="username" value={username} onChange={checkusername} />
            {errorMsg && <div style={{ color: 'red', fontSize: '12px' }}>{errorMsg}</div>}
            <label htmlFor="password">Password<span className='star'>*</span></label>
            <input id="password" type="password" placeholder="Password" value={pass} onChange={validatePassword} />
            {passwordError && <div style={{ color: 'red', fontSize: '12px' }}>{passwordError}</div>}
            <button type="submit" className='addbutton' disabled={!isValidForm()}>Login</button>
          </form>
        </div>
      </div>
    </div>
